...
module i2c_controller #(
    parameter [15:0] CLK_DIV = 16'd35         //desire 400KHz, have 100MHz, thus (1/(400*10^3)*100*10^6)/2, note div by 2 is for need to change in cycle
)(
...
(* X_INTERFACE_INFO = "xilinx.com:interface:gpio:1.0 scl TRI_I" *)
                  input             scl_I,
(* X_INTERFACE_INFO = "xilinx.com:interface:gpio:1.0 scl TRI_O" *)
                  output reg        scl_O,
(* X_INTERFACE_INFO = "xilinx.com:interface:gpio:1.0 scl TRI_T" *)
                  output reg        scl_T,

(* X_INTERFACE_INFO = "xilinx.com:interface:gpio:1.0 sda TRI_I" *)
                  input             sda_I,
(* X_INTERFACE_INFO = "xilinx.com:interface:gpio:1.0 sda TRI_O" *)
                  output reg        sda_O,
(* X_INTERFACE_INFO = "xilinx.com:interface:gpio:1.0 sda TRI_T" *)
                  output reg        sda_T,
...
always@(posedge i_clk or negedge reset_n) begin
    if(!reset_n)
        {clk_i2c_cntr, scl_O} <= 17'b1;
    else if(scl_T)
        {clk_i2c_cntr, scl_O} <= 17'b1;
    else begin
        clk_i2c_cntr <= clk_i2c_cntr + 1;
        if(clk_i2c_cntr == CLK_DIV-1) begin
            scl_O <= !scl_O;
            clk_i2c_cntr <= 0;
        end
    end
...
always@(posedge i_clk or negedge reset_n) begin
    if(!reset_n) begin
        ...
        {addr, rw, sub_addr, sub_len, byte_len} <= 0;
        ...
        case(state)
            ...
            START: begin
                if(scl_prev & scl_curr & clk_i2c_cntr == START_IND_SETUP) begin   //check that scl is high, and that a necessary wait time is held
                    sda_O  <= 1'b0;                                       //set start bit for negedge of clock, and toggle for the clock to begin
                    sda_T  <= 1'b0;
                ...
                if(scl_is_low) begin          //negedge
                    if(clk_i2c_cntr == DATA_HOLD_TIME) begin
                        if(ack_in_prog) begin 
                            sda_O  <= ack_nack;          //write ack until negedge of clk
                            sda_T  <= 1'b0;
                            ack_in_prog <= 1'b0;
                        end
                        else begin
             ...
             */
            RELEASE_BUS: begin
                if(clk_i2c_cntr == CLK_DIV-3) begin
                    scl_T  <= 1'b1;
                    state <= IDLE;
                    sda_T  <= 1'b1;
                    busy <= 1'b0;
